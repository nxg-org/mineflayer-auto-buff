...
export class AutoBuff {
    ...
    private drinking: boolean = false;
    ...
    constructor(public bot: Bot, options: Partial<AutoBuffOptions> = {}) {
        ...
        this.bot._client.on("entity_status", (packet: any) => {
            if (packet.entityId === this.bot.entity.id && packet.entityStatus === 9 && this.drinking) {
                this.drinking = false;
            }
        });
        this.bot.on("entityEffectEnd", async (entity: Entity, effect: mfEffect) => {
            if (entity !== this.bot.entity || !this.effectsToListenFor.includes(this.effects[effect.id].name.toLowerCase())) return;
    ...
    hasItemForBuff(name: string): boolean {
        return (this.findBuffingItems(true)?.length ?? 0) > 0
    }


    findEffectApplyingItems(type: "good" | "bad" | "all", splash: boolean = false) {
        let buffs: mdEffect[];
        if (type !== "all") {
            buffs = Object.values(this.effectsByName).filter((val) => val.type === type);
        } else {
            buffs = Object.values(this.effectsByName);
        }
        let items: Item[];
        if (splash) {
    ...
    findBuffingItems(splash: boolean = false): Item[] | null {
        return this.findEffectApplyingItems("good", splash);
    }

    findDebuffingItems(splash: boolean = false): Item[] | null {
        return this.findEffectApplyingItems("bad", splash);
    }

    findAllEffectItems(): Item[] | null {
        return this.findEffectApplyingItems("bad", true);
    }

    findEffectItems(effect: string): Item[] | null {
        const effectItems = this.findEffectApplyingItems("all", true);
        return effectItems?.filter((item) => (item.nbt?.value as any).Potion.value.includes(effectConversions[effect.toLowerCase()])) ?? null;
    }
    ...
    async applyEffectToSelf(effect: string): Promise<number> {
        ...
        if (items && items.length !== 0) {
            ...
            await this.bot.activateItem(this.useOffHand);
    ...
    async applyEffectsToSelf(...effects: string[]): Promise<number> {
        ...
        for (const effect of effects) {
            ...
            if (items && items.length !== 0) {
                this.drinking = true;
                await this.bot.equip(items[0], hand);
                this.bot.deactivateItem();
                await this.bot.activateItem(this.useOffHand);
                await this.waitUntilFinishedDrinking();
                ran++;
